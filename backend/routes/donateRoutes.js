...
router.post('/request', protect,
    ...
    body('address').trim().isLength({ min: 10, max: 500 }).withMessage('A valid address is required'),
    body('image').matches(/^data:image\/(jpeg|png|webp);base64,/).withMessage('A JPEG, PNG, or WebP image is required'), validate, createDonateRequest);
